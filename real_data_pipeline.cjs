...
async function main() {
  ...
  console.log("Wrote JSON file to src/data/historicalValuationData.json");

  // Create prompt for Ollama
  const filePath = 'C:/Users/gosys/orca/my_pension_helper/src/components/ValuationDcaTab.tsx';
  const code = fs.readFileSync(filePath, 'utf-8');

  const prompt = "You are a senior React developer. We have moved the mock data to a real JSON file. In `ValuationDcaTab.tsx`, please DELETE the `interface WeeklyValuationRow`, DELETE the `generateWeeklyData` function, and DELETE `const HISTORICAL_VALUATION_DATA = generateWeeklyData();`. Instead, add this line at the top with the other imports:\n`import HISTORICAL_VALUATION_DATA from '../data/historicalValuationData.json';`\nReturn ONLY the FULL UPDATED CODE for ValuationDcaTab.tsx enclosed in ```tsx and ```. Do not include any explanations.";

  const payload = JSON.stringify({ model: 'qwen2.5-coder:14b', prompt: prompt, stream: false });
  const reqOptions = { hostname: 'localhost', port: 11434, path: '/api/generate', method: 'POST', headers: { 'Content-Type': 'application/json' } };

  console.log("Sending prompt to local Ollama qwen2.5-coder:14b...");
  const ollamaReq = http.request(reqOptions, (res) => {
    let raw = '';
    res.on('data', c => raw += c);
    res.on('end', () => {
      ...
      if (match && match[1]) {
        ...
        console.log("File updated successfully by local LLM with REAL data imports!");
      } else { 
        console.log("Failed to parse output from LLM."); 
        console.log(output);
      }
    });
  });
